...
async def chat_completions(request_body: ChatCompletionRequest, request: Request):
    ...
    if request_body.session_id:
        try:
            ...
            session = session_store.get_session(request_body.session_id)
            ...
            # frontend's locally-maintained history with the server history.
            incoming_user_message = None
            for msg in reversed(request_body.messages):
                if msg.role == "user" and msg.content:
                    incoming_user_message = msg
                    break
            ...
            if incoming_user_message is not None:
                ...
                request_body.messages.append(incoming_user_message)

                _persist_session_message(
                    session_store,
                    request_body.session_id,
                    "user",
                    incoming_user_message.content,
                )
            ...
            logging.getLogger("openjarvis.server").error(
                "Failed to load persistent session %s",
                request_body.session_id,
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
                detail="Failed to load persistent conversation",
            )
    ...
    # Persist the completed assistant response to the conversational session.
    _persist_session_message(
        session_store,
        request_body.session_id,
        "assistant",
...
def _persist_session_message(
    ...
    """Persist a completed conversational message when a session is active."""
    if session_store is None or not session_id or not content:
        return
