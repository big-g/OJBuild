...
export function InputArea() {
  ...
  const conversationMode = useAppStore(
    (s) => s.settings.conversationMode,
  );

  const conversationModeRef = useRef(conversationMode);
  const voiceInteractionRef = useRef(false);
...
const sendMessage = useCallback(async (messageText?: string) => {
  ...
  if (!selectedModel) {
    ...
    const clientContext = new Intl.DateTimeFormat(clientLocale, {
      dateStyle: 'full',
      timeStyle: 'long',
    ...
    const apiMessages = [{
      ...
      content: [
        ...
        '',
        'CLIENT CONTEXT:',
        `Current local date and time: ${clientContext}`,
        `Client locale: ${clientLocale}`,
...
//            audioMeta = { url: `/api/digest/audio` };
//          }
//        }
//      } catch {
...
//      }
      ...
      if (
        ...
        void speakResponse(accumulatedContent);
      }
  ...
  const handleMicClick = useCallback(async () => {
    ...
    if (speechState === 'recording') {
      ...
      voiceInteractionRef.current = true;
      await startRecording();
    }
  }, [
    speechState,
