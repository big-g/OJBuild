...
fn authenticated_local_client(api_url: &str) -> Result<reqwest::Client, String> {
    let url = reqwest::Url::parse(api_url)
        .map_err(|_| "Invalid OpenJarvis API URL".to_string())?;

    let host = url
        .host_str()
        .ok_or_else(|| "OpenJarvis API URL has no host".to_string())?;

    // The master server API key may ONLY be used with the local
    // OpenJarvis server. Never send it to an arbitrary remote host.
    if host != "127.0.0.1" && host != "localhost" {
        return Err(
            "The OpenJarvis server API key may only be used with the local server"
                .to_string(),
        );
    }

    let api_key = read_server_api_key()
        .ok_or_else(|| "OpenJarvis server API key not found".to_string())?;
    ...
    reqwest::Client::builder()
        .default_headers({
            ...
            .map_err(|_| "Invalid OpenJarvis API key".to_string())?;
...
async fn fetch_energy(api_url: String) -> Result<serde_json::Value, String> {
    ...
    let client = authenticated_local_client(&base)?;
    let resp = client
        .get(format!("{}/v1/telemetry/energy", base))
        .send()
        .await
        .map_err(|e| format!("Connection failed: {}", e))?;
...
async fn fetch_telemetry(api_url: String) -> Result<serde_json::Value, String> {
    ...
    let client = authenticated_local_client(&base)?;
    let resp = client
        .get(format!("{}/v1/telemetry/stats", base))
        .send()
        .await
        .map_err(|e| format!("Connection failed: {}", e))?;
...
async fn fetch_traces(api_url: String, limit: u32) -> Result<serde_json::Value, String> {
    ...
    let client = authenticated_local_client(&base)?;
    let resp = client
        .get(format!("{}/v1/traces?limit={}", base, limit))
        .send()
        .await
        .map_err(|e| format!("Connection failed: {}", e))?;
...
async fn fetch_trace(api_url: String, trace_id: String) -> Result<serde_json::Value, String> {
    ...
    let client = authenticated_local_client(&base)?;
    let resp = client
        .get(format!("{}/v1/traces/{}", base, trace_id))
        .send()
        .await
        .map_err(|e| format!("Connection failed: {}", e))?;
...
async fn fetch_learning_stats(api_url: String) -> Result<serde_json::Value, String> {
    ...
    let client = authenticated_local_client(&base)?;
    let resp = client
        .get(format!("{}/v1/learning/stats", base))
        .send()
        .await
        .map_err(|e| format!("Connection failed: {}", e))?;
...
async fn fetch_learning_policy(api_url: String) -> Result<serde_json::Value, String> {
    ...
    let client = authenticated_local_client(&base)?;
    let resp = client
        .get(format!("{}/v1/learning/policy", base))
        .send()
        .await
        .map_err(|e| format!("Connection failed: {}", e))?;
...
/// Read the OpenJarvis server API key from config.toml.
/// The key remains inside the Tauri process and is never exposed to the UI.
fn read_server_api_key() -> Option<String> {
...
async fn speech_health(api_url: String) -> Result<serde_json::Value, String> {
    ...
    let url = format!("{}/v1/speech/health", base);
